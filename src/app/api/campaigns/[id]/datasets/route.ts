...
import { datasetSelectionSchema } from "@/server/validation/campaign-schema";
import {
  setCampaignDatasets,
...
export async function PUT(
  ...
  const parsed = datasetSelectionSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json({ error: parsed.error.flatten() }, { status: 400 });
  }

  try {
    const campaign = await setCampaignDatasets(id, parsed.data.datasetIds, actor);
