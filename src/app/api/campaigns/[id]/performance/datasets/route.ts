...
import { getDatasetPerformance } from "@/server/services/campaign-performance-service";
...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const datasets = await getDatasetPerformance(id);
    return NextResponse.json({ campaignId: id, datasets });
  } catch (err) {
